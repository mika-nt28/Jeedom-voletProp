...
class voletProp extends eqLogic {
	public static function timeout($_option) {	
		...
		if (is_object($Volet) && $Volet->getIsEnable()) {
			while(true){
				...
				if(cache::byKey('voletProp::Move::'.$Volet->getId())->getValue(false) && cache::byKey('voletProp::Prop::'.$Volet->getId())->getValue(false)){
			...
			if($Volet->getConfiguration('useStateJeedom') || $Volet->getConfiguration('useStateManual')){	
				...
				if($Volet->EvaluateCondition($isUp)){
					if($Volet->getConfiguration('StopStateCmd') == '' && cache::byKey('voletProp::Move::'.$Volet->getId())->getValue(false) && cache::byKey('voletProp::ChangeState::'.$Volet->getId())->getValue(false)){
						...
						cache::set('voletProp::Prop::'.$Volet->getId(),false, 0);
	...
	public function StopListener() {
		...
		if (is_object($cache))
			$cache->remove();
		$cache = cache::byKey('voletProp::Prop::'.$this->getId());
		if (is_object($cache))
			$cache->remove();
    ...
    public function execute($_options = null) {
		switch($this->getLogicalId()){
			case "up":
				...
				cache::set('voletProp::Prop::'.$this->getEqLogic()->getId(),false, 0);
			...
			case "position":
				if(!cache::byKey('voletProp::Move::'.$this->getEqLogic()->getId())->getValue(false)){
					...
					cache::set('voletProp::Prop::'.$this->getEqLogic()->getId(),true, 0);
				}
			break;
		}
	}
